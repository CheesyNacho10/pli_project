(define (domain robots)
  ...
  (:predicates
    ...
    (at ?i - robot ?r - room)
    (at ?i - person ?r - room)
    (at ?i - cupholder ?r - room)
    (at ?i - tea-machine ?r - room)
    (hand-state EH - state ?h - hand ?rob - robot)
    (hand-state EC - state ?h - hand ?rob - robot)
    (hand-state FC - state ?h - hand ?rob - robot)
    (served ?p - person)
  )
